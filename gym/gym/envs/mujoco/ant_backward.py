...
class AntBackwardEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
    ...
    def _step(self, a):
        ...
        backward_reward = -1 * forward_reward
        ...
        # Check the position of head and torso
        #notdone = np.isfinite(state).all() \
        #      and self.data.geom_xpos[2][1] - self.data.geom_xpos[1][1] > -0.3
        done = not notdone
        ob = self._get_obs()
