...
class HumanoidMovingFloorEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        ...
        mujoco_env.MujocoEnv.__init__(self, 'humanoid_moving_floor.xml', 5)
    ...
    def _put_floor(self, init=False):
        ...
        if init or not hasattr(self, "extra_obs"):
            ...
            idx = np.where(['moving_floor' in name.decode() for name in self.model.geom_names])[0][0]
            size_x = geom_size[idx][0]
            pos_x = prev_pos_x + prev_size_x + size_x + abs(self.model.jnt_range[-1][0])
            geom_pos[idx][0] = pos_x 
            '''
            for i in range(1, 4):
    ...
    def _move_the_floor(self):
        qvel = self.model.data.qvel.copy()
        qpos = self.model.data.qpos.copy()
        if (self.timestep // 1000) % 2 == 0:
            qvel[-1] = 1
        elif (self.timestep // 1000) % 2 == 1:
            qvel[-1] = -1
        else:
            print("Error")
        self.set_state(np.squeeze(qpos), np.squeeze(qvel))

    def _step(self, a):

        pos_before = mass_center(self.model)
        self._move_the_floor()
        ...
        x_vel_reward = self._config["x_vel_weight"] * min(self._vel, 4)
